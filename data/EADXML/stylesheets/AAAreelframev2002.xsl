<!-- stephreelframe generated by Steph 6/05 using Loren's stylesheet generated by Loren 1/31/02 from E AD Cookbook Style 2 and Michael Fox's changes to the container list Version 0.9   2000 July 22; incorporating changes attempting to accomodate EAD Cookbook v. 2  -->
<!--  This stylesheet generates Style 2 and Michael Fox's addition for reels and frames which has a Table of Contents in an HTML table cell along the left side of the screen. It may be used when an HTML frame is not desired.  -->
...
<!-- changed these variables to refer to the nodes, not hold
     their values - wap -->
  <xsl:variable name="reelnumber" select="container[@type='reel']"/>
  <xsl:variable name="prevreelnumber" select="preceding::container[@type='reel'][1]"/>
  <xsl:variable name="framenumber" select="container[@type='frame']"/>
...
<!--Within the cases where c02 is a file, there are two further options
for display.-->

<xsl:choose>

<!--The first option selects cases where either the value of the reel container or the frame
container is the same as that of a previous component.   In that case, no column labels are added and only the frame number appears
along with the other component data.-->

<xsl:when test="not($reelnumber = $prevreelnumber) or (not(../preceding-sibling::c02[1]) and ((../../@level) or (../../@otherlevel)))" >

	<!--Selects cases where the value of the reel or frame container is not the same as
that of a previous component.   In that case, column labels are added and both the
reel and frame numbers appear along with the other component data.-->
...
<xsl:text>&#160;</xsl:text>
  ...
  <xsl:otherwise>
	...
	<xsl:choose> 
		<xsl:when test="not($reelnumber = $prevreelnumber) or (not(../preceding-sibling::c03[1]) and ((../../@level) or (../../@otherlevel)))">
        
      <!--Selects cases where the value of the reel container is not the same as that of a previous
      component.   In that case, column labels are added and both the reel and frame number appears
      along with the other component data.-->
...
<xsl:choose>
  ...
  <xsl:otherwise>
		...
		<xsl:when test="not($reelnumber = $prevreelnumber) or (not(../preceding-sibling::c04[1]) and ((../../@level) or (../../@otherlevel)))">
      <!--Selects cases where the value of the reel container is not the same as that of a previous
      component.   In that case, column labels are added and both the reel and frame number appears
      along with the other component data.-->
...
<xsl:choose>
  ...
  <xsl:otherwise>
    ...
    <xsl:choose>
      <xsl:when test="not($reelnumber = $prevreelnumber) or (not(../preceding-sibling::c05[1])  and ((../../@level) or (../../@otherlevel)))" >
