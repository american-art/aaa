...
<!-- changed these variables to refer to the nodes, not hold
  ...
  <xsl:variable name="foldernumber" select="container[@type='folder']"/>
...
<xsl:when test="not($boxnumber = $prevboxnumber) or (not(../preceding-sibling::c02[1]) and ((../../@level) or (../../@otherlevel)))" >
...
<xsl:text>&#160;</xsl:text>
  ...
  <xsl:otherwise>
	...
	<xsl:choose> 
		<xsl:when test="not($boxnumber = $prevboxnumber) or (not(../preceding-sibling::c03[1]) and ((../../@level) or (../../@otherlevel)))">
...
<xsl:choose>
  ...
  <xsl:otherwise>
		...
		<xsl:when test="not($boxnumber = $prevboxnumber) or (not(../preceding-sibling::c04[1]) and ((../../@level) or (../../@otherlevel)))">
      <!--Selects cases where the value of the box container is not the same as that of a previous
      component.   In that case, column labels are added and both the box and folder number appears
      along with the other component data.-->
...
<xsl:choose>
  ...
  <xsl:otherwise>
    ...
    <xsl:choose>
      <xsl:when test="not($boxnumber = $prevboxnumber) or (not(../preceding-sibling::c05[1])  and ((../../@level) or (../../@otherlevel)))" >
